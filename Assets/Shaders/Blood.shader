Shader "Universal Render Pipeline/2D/Blood" {
    Properties {
        ...
        Pass {
            ...
            */
            ...
            float4 frag(Varyings v) : SV_Target {
                ...
                float h1 = (snoise(float2(v.uv.x + noise1D(_SinTime.x), v.uv.y + noise1D(_CosTime.y))) * noise1D(_Time.x) * 0.5 + 0.5);
                float h2 = unity_gradientNoise(v.uv + _SinTime.x).x;
